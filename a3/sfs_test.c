 ...
 mksfs(1);                     /* Initialize the file system. */
  ...
  for (i = 0; i < 2; i++) {
    for (j = 0; j < filesize[i]; j += chunksize) {
	  ...
	  printf("ERROR1: data error at offset %d in file %s (is %d,should be %d)\n",
                  j+k, names[i], buffer[k], (char)(j+k));
	 
          fprintf(stderr, "ERROR: data error at offset %d in file %s (is %d,should be %d)\n",
                  j+k, names[i], buffer[k], (char)(j+k));
          error_count++;
          break;
        }
  ...
  for (j = 0; j < strlen(test_str); j++) {
    for (i = 0; i < nopen; i++) {
      ...
      if (ch != test_str[j]) {
	printf("ERROR: Read wrong byte from %s at %d (%d,%d)\n",
  ...
  for (i = 0; i < nopen; i++) {
	...
	fprintf(stderr, "ERROR: Read wrong number of bytes\n");
	  ...
	  printf("Error: wrong byte is in %s at index %d (is %d, should be %d)\n", names[i],j,fixedbuf[j],test_str[j]);
